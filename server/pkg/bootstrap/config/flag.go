...
func (f *CommandFlags) defineFlag() {
	...
	flag.StringVar(&f.ConfigHost, "conf-host", "127.0.0.1:8500", "config server host, eg: -conf-host 127.0.0.1:8500")
	flag.StringVar(&f.ConfigType, "conf-type", "consul", "config server host, eg: -conf-type consul")
...
func (f *CommandFlags) beDaemon(arg string) {
	f.subProcess(f.stripSlice(os.Args, arg))
	fmt.Printf("[*] Daemon running in PID: %d PPID: %d\n", os.Getpid(), os.Getppid())
	os.Exit(0)
}

func (f *CommandFlags) stripSlice(slice []string, element string) []string {
	for i := 0; i < len(slice); {
...
func (f *CommandFlags) subProcess(args []string) *exec.Cmd {
